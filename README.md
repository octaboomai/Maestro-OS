...
- **🎯 Market Intelligence (DealScout)** — B2B market & competitor intelligence swarm that outputs a structured dashboard (`dealscout_engine.py`)
...
5. In the **Secrets** field, paste the same flat content as `secrets.toml.example`, with real values:
   ...
   ```
   Keep these at the root level with no `[section]` header — Streamlit only mirrors root-level secrets into `os.environ`, which is how both engine files read them.
